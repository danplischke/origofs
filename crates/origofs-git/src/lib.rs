//! origofs-git — drive an origofs workspace with the real `git` (`docs/DESIGN.md` §4c,
//! the git-interop layer; roadmap M5).
//!
//! origofs stays BLAKE3-native internally; this crate bridges its opt-in commit DAG
...
//!
//! The `origofs-remote-git` crate builds on this to provide `git-remote-origofs`, so the
