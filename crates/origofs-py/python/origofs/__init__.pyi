...
class PathRevision(TypedDict):
    """One commit that changed a single path (``log_path``).

    ``status`` is ``A``/``M``/``D``, and ``hash`` is the path's content address at
    that commit -- ``None`` when the commit deleted it.
    """
