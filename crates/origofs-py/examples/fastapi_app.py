"""Example: driving an origofs workspace from FastAPI.
...
write to that user/agent via `WriteCtx`. origofs records blame + an edit-op audit
...
WS: origofs.Workspace  # set on startup


@asynccontextmanager
async def lifespan(app: FastAPI):
    global WS
    # Local dev; swap for origofs.Workspace.open_pg(dsn, cas_dir) for multi-writer.
...
# --- identity injection -----------------------------------------------------
# Replace this with your real auth. Resolve the request's principal to an origofs
...
async def write_file(path: str, body: bytes, ctx: origofs.WriteCtx = Depends(current_ctx)):
...
async def suggest(path: str, body: bytes, summary: Optional[str] = None,
                  ctx: origofs.WriteCtx = Depends(current_ctx)):
...
async def accept(sid: int, ctx: origofs.WriteCtx = Depends(current_ctx)):
    try:
        await WS.accept_suggestion(sid, ctx)  # applied, credited to the agent
    except origofs.ConflictError as e:
...
#
#   mount = WS.mount("/mnt/origofs")          # FUSE, returns a handle
#   ...
#   mount.unmount()
#
#   import asyncio
