...
- **The `Propose` write policy is enforced in the engine, not per surface.**
  ...
  exist for internal machinery (checkout, merge materialization, applying an
  accepted suggestion), and are exempt by construction. `tests/mcp.rs` fails on an unclassified MCP tool
